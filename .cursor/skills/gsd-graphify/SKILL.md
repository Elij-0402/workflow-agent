...
**CJS-only (graphify):** `graphify` subcommands are not registered on `gsd-sdk query`. Use `node D:/workflow-agent/.cursor/get-shit-done/bin/gsd-tools.cjs graphify …` as documented in this command and in `docs/CLI-TOOLS.md`. Other tooling may still use `gsd-sdk query` where a handler exists.
...
Knowledge graph is disabled. To activate:
  ...
  node D:/workflow-agent/.cursor/get-shit-done/bin/gsd-tools.cjs config-set graphify.enabled true
...
```bash
node D:/workflow-agent/.cursor/get-shit-done/bin/gsd-tools.cjs graphify query <term>
```
...
```bash
node D:/workflow-agent/.cursor/get-shit-done/bin/gsd-tools.cjs graphify diff
```
...
```bash
node "D:/workflow-agent/.cursor/get-shit-done/bin/gsd-tools.cjs" graphify build
```
...
graphify update . \
  ...
  && node "D:/workflow-agent/.cursor/get-shit-done/bin/gsd-tools.cjs" graphify build snapshot \
  && node "D:/workflow-agent/.cursor/get-shit-done/bin/gsd-tools.cjs" graphify status
```
