...
export function CostEstimateModal({ open, chapterCount, avgChars, onCancel, onConfirm }: Props) {
  if (!open) return null;
  ...
  return (
    <div className="bg-background/72 fixed inset-0 z-40 flex items-center justify-center backdrop-blur-[2px]">
      <div className="surface-panel max-w-md space-y-4 bg-card p-6">
        <div>
          <p className="eyebrow-label">cost preview</p>
          <h3 className="mt-2 font-display text-[22px] italic leading-tight text-foreground">
            本次将发起 {est.calls} 次 LLM 调用
          </h3>
        </div>

        <pre className="overflow-x-auto whitespace-pre rounded-[2px] border border-dashed border-border/70 bg-background/40 p-4 font-mono text-[12px] leading-7 text-muted-foreground">
