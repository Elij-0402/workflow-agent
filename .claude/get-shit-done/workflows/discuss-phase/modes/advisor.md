...
> `D:/workflow-agent/.claude/get-shit-done/USER-PROFILE.md` exists). Skip the Read
...
```bash
PROFILE_CONTENT=$(cat "D:/workflow-agent/.claude/get-shit-done/USER-PROFILE.md" 2>/dev/null || true)
```
...
2. For EACH user-selected gray area, spawn a `Agent()` in parallel:
   ...
   ```
   Agent(
     prompt="First, read @D:/workflow-agent/.claude/agents/gsd-advisor-researcher.md for your role and instructions.
