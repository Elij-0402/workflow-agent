...
@D:/workflow-agent/.claude/get-shit-done/references/git-integration.md
</required_reading>
...
See `D:/workflow-agent/.claude/get-shit-done/references/tdd.md` for structure.
</tdd_plan_execution>
...
See D:/workflow-agent/.claude/get-shit-done/references/checkpoints.md for details.
</step>
...
If user_setup exists: create `{phase}-USER-SETUP.md` using template `D:/workflow-agent/.claude/get-shit-done/templates/user-setup.md`. Per service: env vars table, account setup checklist, dashboard config, local dev notes, verification commands. Status "Incomplete". Set `USER_SETUP_CREATED=true`. If empty/missing: skip.
</step>
...
Create `{phase}-{plan}-SUMMARY.md` at `.planning/phases/XX-name/`. Use `D:/workflow-agent/.claude/get-shit-done/templates/summary.md`.
