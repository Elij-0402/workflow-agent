...
- **Global scope**: `D:/workflow-agent/.claude/notes/{YYYY-MM-DD}-{slug}.md` — fallback when no `.planning/`, or when `--global` flag is present
...
2. Ensure the notes directory exists (`.planning/notes/` or `D:/workflow-agent/.claude/notes/`)
...
2. Glob `D:/workflow-agent/.claude/notes/*.md` (if directory exists) — global notes
...
Global (D:/workflow-agent/.claude/notes/):
...
2. **No `.planning/`**: Falls back to global `D:/workflow-agent/.claude/notes/` — works in any directory
