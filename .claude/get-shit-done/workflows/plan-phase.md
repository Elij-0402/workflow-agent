...
@D:/workflow-agent/.claude/get-shit-done/references/ui-brand.md
@D:/workflow-agent/.claude/get-shit-done/references/revision-loop.md
@D:/workflow-agent/.claude/get-shit-done/references/gate-prompts.md
@D:/workflow-agent/.claude/get-shit-done/references/agent-contracts.md
@D:/workflow-agent/.claude/get-shit-done/references/gates.md
</required_reading>
...
When `TDD_MODE` is `true`, the planner agent is instructed to apply `type: tdd` to eligible tasks using heuristics from `references/tdd.md`. The planner's `<required_reading>` is extended to include `@D:/workflow-agent/.claude/get-shit-done/references/tdd.md` so gate enforcement rules are available during planning.
...
- Planner is instructed to produce `SKELETON.md` in the phase directory alongside `PLAN.md`. The template lives at `@D:/workflow-agent/.claude/get-shit-done/references/skeleton-template.md`.
...
**If found:**
1. Read template: `D:/workflow-agent/.claude/get-shit-done/templates/VALIDATION.md`
...
${TDD_MODE === 'true' ? `
<tdd_mode_active>
**TDD Mode is ENABLED.** Apply TDD heuristics from @D:/workflow-agent/.claude/get-shit-done/references/tdd.md to all eligible tasks:
...
**MVP_MODE:** ${MVP_MODE} (when true, follow vertical-slice rules from `@D:/workflow-agent/.claude/get-shit-done/references/planner-mvp-mode.md`; when false, ignore MVP guidance entirely.)
...
${MVP_MODE === 'true' ? `
<mvp_mode_active>
**MVP Mode is ENABLED.** Follow vertical-slice planning rules from @D:/workflow-agent/.claude/get-shit-done/references/planner-mvp-mode.md. Each plan must deliver a complete vertical slice — thin end-to-end functionality rather than horizontal layers.
</mvp_mode_active>
` : ''}
</planning_context>
...
if [ "$POST_PLANNING_GAPS" = "true" ]; then
  node "D:/workflow-agent/.claude/get-shit-done/bin/gsd-tools.cjs" gap-analysis --phase-dir "${PHASE_DIR}"
fi
```
