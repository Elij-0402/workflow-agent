...
test.describe("dual upload branches", () => {
  ...
  test("blocks submission when only one file is selected", async ({ page }) => {
    ...
    await expect(
      page.getByText(path.basename(singleFile), { exact: false }),
    ).toBeVisible();
    await expect(
  ...
  test("blocks submission when three files are selected", async ({ page }) => {
    await page
      ...
      .setInputFiles([tripleA, tripleB, tripleC]);
  ...
  test("allows submission when exactly two files are selected", async ({
    page,
  }) => {
    await page.locator('input[type="file"]').setInputFiles([fileA, fileB]);

    await expect(
      page.getByRole("heading", { name: "参考书 1", exact: true }),
    ).toBeVisible();
    await expect(
      page.getByRole("heading", { name: "参考书 2", exact: true }),
    ).toBeVisible();
    await expect(
      page.getByText(path.basename(fileA), { exact: false }),
    ).toBeVisible();
    await expect(
      page.getByText(path.basename(fileB), { exact: false }),
    ).toBeVisible();
